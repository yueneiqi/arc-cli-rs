...
async fn main() {
    ...
    let result = match command {
        Commands::ArcVersion => match arc::get_version().await {
            Ok(version) => {
        ...
        Commands::ListSpaces => match arc::get_spaces().await {
            Ok(spaces) => {
                println!("Arc spaces:\n");
                for space in spaces {
        ...
        Commands::SelectSpace { space_id } => arc::select_space(space_id).await,
        Commands::SelectSpaceName {
            space_name,
            case_insensitive,
            partial,
        } => {
        ...
        Commands::ListTabs => match arc::get_tabs().await {
            Ok(tabs) => {
                println!("Arc tabs:\n");
                for tab in tabs {
                    println!(
        ...
        Commands::SelectTab { window_id, tab_id } => arc::select_tab(window_id, tab_id).await,
        Commands::SelectTabName {
            tab_name,
            case_insensitive,
            partial,
        } => {
        ...
        Commands::ReloadTab { window_id, tab_id } => arc::reload_tab(window_id, tab_id).await,
        Commands::CloseTab { window_id, tab_id } => arc::close_tab(window_id, tab_id).await,
